...
read -p "Ingrese el nombre de usuario predeterminado [predeterminado admin]： " un
read -p "Ingrese la contraseña de inicio de sesión predeterminada [predeterminada admin]： " pw
read -p "Ingrese el número de puerto de escucha [predeterminado 5000]： " uport
if [[ -z "${uport}" ]];then
	uport="5000"
else
	if [[ "$uport" =~ ^(-?|\+?)[0-9]+(\.?[0-9]+)?$ ]];then
		if [[ $uport -ge "65535" || $uport -le 1 ]];then
			echo "Valor de rango de puerto[1,65535]，Número de puerto predeterminado de la aplicación 5000"
			unset uport
			uport="5000"
		else
			tport=`netstat -anlt | awk '{print $4}' | sed -e '1,2d' | awk -F : '{print $NF}' | sort -n | uniq | grep "$uport"`
			if [[ ! -z ${tport} ]];then
				echo "¡El número de puerto ya existe! Número de puerto predeterminado de la aplicación 5000"
				unset uport
				uport="5000"
			fi
		fi
	else
		echo "Por favor ingrese los números! Número de puerto predeterminado de la aplicación 5000"
		uport="5000"
	fi
fi
if [[ -z "${un}" ]];then
...
"
echo "Puerto del panel：${uport}"
echo "Nombre de usuario predeterminado：${un}"
echo "Contraseña predeterminada：${pw}"
echo ''
echo "Ingrese v2ray y presione Enter para administrar manualmente las funciones relacionadas del panel web"
